...
def _minimal_response(**overrides) -> PredictionResponse:
    base = dict(
...
def test_predict_payload_carries_currency():
...
def test_resolve_currency_defaults_to_usd():
...
def test_resolve_currency_swallows_fx_errors():
...
def test_predict_response_defaults():
