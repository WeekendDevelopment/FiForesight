...
def _es256_token(private_key, sub=_UUID, kid="test-kid") -> str:
    return jwt.encode(
...
def test_es256_token_wrong_key_rejected(monkeypatch):
...
def test_es256_token_without_jwks_fails_closed(monkeypatch):
...
def test_hs256_token_still_verified(monkeypatch):
...
def test_non_bearer_returns_empty():
