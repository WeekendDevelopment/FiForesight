...
from calibration_service import MIN_CALIBRATION_SAMPLES, _compute_calibration
    ...
    samples = [
        {
            ...
            "low": 95.0,
            "high": 105.0,
            "actual": 100.0 if i < 8 else 130.0,
        }
        for i in range(10)
    ]
    ...
    samples = [
        {
            ...
            "low": 99.5,
            "high": 100.5,
            "actual": 100.0 if i == 0 else 115.0,
        }
        for i in range(10)
    ]
    records, outcomes = _build(samples)
...
def test_edge_vs_naive() -> None:
    ...
    samples = [{"prev": 105.0, "last": 100.0, "e_d1": 103.0, "actual": 104.0} for _ in range(6)]
    records, outcomes = _build(samples)
    r = _compute_calibration(records, outcomes)
...
def test_insufficient_history() -> None:
    ...
    samples = [
        {"last": 100.0, "e_d1": 101.0, "low": 95.0, "high": 105.0, "actual": 100.0}
        for _ in range(n)
    ]
    records, outcomes = _build(samples)
    r = _compute_calibration(records, outcomes)
...
def test_calibration_endpoint_all_aggregate() -> None:
    samples = [
        {"last": 100.0, "e_d1": 100.0, "low": 95.0, "high": 105.0, "actual": 100.0}
        for _ in range(6)
    ]
    records, outcomes = _build(samples)
    mock = MagicMock()
