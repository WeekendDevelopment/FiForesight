...
│   │   ├── market.py           # /dcf /options /ipo/calendar /earnings/calendar /sectors/heatmap (F23) /sectors/rotation (F27) /briefing /orderbook /macro/snapshot /insider/{symbol} (F15) /screener (F24) /market/treemap (F32) /symbols/search (F33)
...
│   ├── fx.py                   # get_usd_rate() — keyless yfinance FX pairs (GBPUSD=X…), GBp/minor-unit aware, Redis 1h (F35)
...
│   │   ├── CommandPalette.tsx                # Global Ctrl/Cmd+K palette — tickers/pages/actions (F33)
│   │   ├── DCFCard.tsx                       # 3-scenario DCF valuation (bear/base/bull)
│   │   ├── FundamentalsPanel.tsx             # Extended metrics panel
│   │   ├── MarketTreemap.tsx                 # Finviz-style cap-weighted stock treemap (F32, /sectors "Map" view)
...
│   ├── lib/currency.ts                       # formatPrice/convertToUsd — NEVER hardcode '$' on a price (F35)
│   ├── lib/navItems.ts                       # SHARED nav model — sidebar + mobile nav + palette (F33)
│   ├── lib/tickerSearch.ts                   # SHARED ticker universe + fuzzy search (F33)
│   ├── lib/fuzzy.ts                          # Pure fuzzy matcher (prefix > word-start > subsequence)
│   └── ...config files
├── .claude/
  ...
  → Frontend /api/predict (Next.js proxy)
    → FastAPI /predict (routers/predict.py)
      ...
      → Response: history, fundamentals, indicators, forecasts, jury verdicts, news, sentiment, monte carlo,
        ...
        currency + fxToUsd (F35 — quote currency from info['currency'] + 1-unit→USD multiplier, null on FX failure)
...
Market treemap flow (Feature 32 — "Map" view toggle on /sectors, public):
  GET /api/market/treemap → List[{symbol, name, sector, marketCap, changePct}]
    → universe/fundamentals from the screener snapshot (reuses the 1h "screener:universe"
      Redis key; builds + caches it on a cold cache exactly like /screener)
    → fresh 1d % change from ONE batched yf.download(period="5d", group_by="ticker")
    → ETF proxies (SPY/QQQ/IWM) + rows without marketCap excluded; sorted marketCap desc
    → a symbol missing from the price batch keeps changePct=null (grey tile), never a 500
    → whole result Redis-cached 10 min; 502 on total failure. 60/min (read-only tier).
```
...
| Currency-aware price display — formatPrice everywhere, /predict currency+fxToUsd, "Show in USD" toggle (F35) | pending |
...
- **Market Treemap (Feature 32)** — the `/sectors` page now has **two views sharing one route**: a "Sectors | Map" `ToggleButtonGroup` in the page header (persisted in localStorage **`fiforesight:sectors:view`**, lazy `useState` init) switches between the F23 `SectorHeatmapPanel` and `MarketTreemap.tsx` — deliberately NO new nav item. `GET /market/treemap` (`routers/market.py`, read-only tier, Redis 10 min under `market:treemap`) **reuses the screener's `screener:universe` 1h snapshot** (never builds a second universe; populates that same key on a cold cache exactly like `/screener`) for symbol/name/sector/marketCap, then fetches fresh 1d % changes with ONE batched `yf.download(period="5d", group_by="ticker")` (grouped/flat column handling mirrors the heatmap's `_closes_for`). ETF proxies (SPY/QQQ/IWM) and capless rows are excluded (cap-weighting is meaningless for them); a symbol missing from the price batch keeps `changePct: null` (grey tile) rather than 500ing; rows sorted cap-desc; 502 only on total failure. Frontend: `MarketTreemap.tsx` (Recharts `<Treemap>`, nested sector→stock data so sectors form blocks, same ±0.5/±2% palette thresholds as the heatmap, custom `content` tile renderer, tooltip with compact cap, click → `/analysis?symbol=`; heights 380/520/640 via `useMediaQuery`). **Gotchas**: SVG paints children over parents, so a depth-1 (sector) label would be hidden under the leaf tiles — the sector caption is instead drawn inside each sector's largest tile (`sectorLabel` flag on the first child); tile labels self-drop by computed pixel size (>55×28 symbol+%, >34×16 symbol only, else none), which is what keeps the map legible at 320px. Tests in `test_market_treemap.py` (mock `build_universe_snapshot` + `yf.download`).
...
- **Analysis Navigator (Feature 34)** — the analysis page's left-column cards are grouped into six collapsible `AnalysisSection` wrappers (`overview/forecast/jury/trade/valuation/data`) with a sticky `AnalysisSectionNav` chip row (IntersectionObserver active-tracking, smooth jump-to via `scroll-margin-top`, collapse state persisted as one JSON object under localStorage **`fiforesight:analysis:sections`**). **Gotchas**: (1) a NEW card on the analysis page must be added *inside* one of the sections, not as a loose `Stack` child; (2) collapse must NEVER unmount cards — several self-fetch on mount (`StockReportCard`, `DividendIncomeCard`, `OrderBookPanel`, `BacktestPanel`, `AnalystTargetsCard`…), so `AnalysisSection` uses MUI `<Collapse>` WITHOUT `unmountOnExit` (children stay mounted; expanding fires zero new requests) — don't "optimize" that away; (3) the right-column sidebar (Fundamentals, Order Book, Peers, news) is deliberately OUTSIDE the sections — on desktop it sits beside the content, so folding it into a vertical section would relocate it. The nav renders only once a prediction is loaded, so logged-out layout specs are unaffected.
- **Currency-aware price display (Feature 35)** — every price the app shows carries the instrument's REAL quote currency; **never hardcode `$` on a price — render through `formatPrice(value, currency)`** (`frontend/lib/currency.ts`; minor units render as a suffix: yfinance quotes LSE in **GBp = pence**, so BP.L is "516.50p", NOT "$516.50" — wrong symbol AND ~75× off; JSE `ZAc`/TASE `ILA` likewise). **Backend**: `backend/fx.py::get_usd_rate(currency)` returns the 1-quote-unit→USD multiplier via keyless yfinance FX pairs (`GBPUSD=X`…; minor units scale ×0.01), Redis-cached 1h per currency (`fx:usd:{cur}`), `None` on failure — callers degrade to native display, never guess. `/predict` adds top-level `currency` (default "USD") + `fxToUsd` (nullable) — resolved from the fetch_info metrics by `_resolve_currency`, never breaks /predict. Screener/watchlist-metrics/treemap rows carry a label-only `currency` field (no per-row conversion in v1; a pre-F35 cached `screener:universe` snapshot yields `currency: null` — handled). **Frontend**: the analysis page owns a **"Show in USD" toggle** (renders only when `currency !== 'USD' && fxToUsd != null`; persisted under localStorage **`fiforesight:currency:usd`**; live-rate caption "1 GBP ≈ $1.27") and passes the active `currency` + `fx` display-multiplier props to every price card (PriceChartCard/TradeSetup/DayTrade/DCF/AnalystTargets/Fundamentals/MC fan chart/BacktestPanel) — conversion happens at DISPLAY time only; underlying state, stored data, and portfolio cost basis stay native/as-entered. Chart Y-axes stay numeric except labelled affixes (`currencyAffixes`). Deliberately still `$`: SEC Form 4 insider prices, US options chains, US IPO ranges (genuinely USD); PeerComparisonPanel awaits per-peer currency. 10 tests in `test_fx.py`.
- **Command Palette (Feature 33)** — global Linear/Notion-style launcher: **Ctrl/Cmd+K** toggles it from anywhere (even with an input focused); **"/"** opens it only when no input/textarea/contentEditable has focus; **Esc** closes. `CommandPalette.tsx` (MUI Dialog + InputBase + listbox — deliberately **no cmdk/kbar dependency**) is rendered ONCE by the (app) layout's `Shell` and self-manages open state; other UI opens it via the exported `openCommandPalette()` (a window CustomEvent, `fiforesight:palette:open` — no prop threading). Three result groups share one keyboard-navigable `role="listbox"` (ArrowUp/Down wrap across ALL groups, `aria-activedescendant` on the input): **Tickers** (static universe first, then **live multi-exchange search** — see below) → `/analysis?symbol=`, **Pages**, **Actions** (theme toggle via `AppShellContext`, quick-jumps, random ticker); ticker rows also carry a **watchlist add/remove Chip** (signed-in only, `stopPropagation` so the palette stays open). **Live symbol search**: `GET /symbols/search?q=` (`routers/market.py`, read-only tier, Redis 6h per query) proxies Yahoo's keyless search endpoint and returns every listing of a name across exchanges (`BP` NYSE, `BP.L` London, `AKRBP.OL` Oslo…) as `{symbol, name, exchange, type}` — carets (`^GSPC`) and OPTION/MUTUALFUND types filtered, `[]` on any upstream failure (never 500; the palette degrades to its static list + an `Analyze "Q"` free-type fallback row). The palette debounces 250ms (min 2 chars, AbortController per keystroke) via `searchSymbolsRemote` in `lib/tickerSearch.ts` and renders an exchange badge per live row; the Next proxy (`app/api/symbols/search/route.ts`) is deliberately **`cache: 'no-store'`** (backend Redis already caches; a Next `revalidate` would pin transient-failure `[]` for the whole window — this bit us in QA). **The analysis AND landing pages' inline Autocompletes + manual exchange dropdowns were REMOVED** — their search bars are now palette triggers (`data-testid="analysis-search-trigger"` / `"home-search-trigger"`), and the sidebar's separate Search button was dropped as redundant (entry points: Ctrl+K, the two page triggers, the mobile bottom-nav Search button): exchange selection happens by picking the Yahoo-suffixed listing (`BP.L`), which `/predict` accepts natively — the old `:LSE` suffix was cosmetic (`_to_yf_symbol` strips `":EXCHANGE"`, so it always fetched the US listing anyway). Empty query shows **Recent tickers** — last 5 chosen symbols in localStorage **`fiforesight:palette:recent`**. **Shared modules (edit these, NOT inline copies)**: `lib/navItems.ts` holds `NAV_ITEMS` (sidebar + mobile nav + palette all import it; the old inline array in `layout.tsx` is gone — keep `tests/e2e/responsive.spec.ts` ROUTES in sync), and `lib/tickerSearch.ts` holds the ticker universe (`TICKER_UNIVERSE` with company names, `POPULAR_TICKERS` symbol list) — the duplicated inline lists on the analysis/options/landing pages were removed (only the options page still consumes `POPULAR_TICKERS` via a plain Autocomplete; options chains are US-listing data). Fuzzy ranking lives in `lib/fuzzy.ts` (`matchTier`: prefix > word-start > subsequence; pure, dependency-free). Responsive: desktop = centered 560px modal; mobile (`down('md')`) = full-width top sheet opened from a **Search button in the bottom nav** (`data-testid="palette-search-button"`), 17px input (no iOS zoom), 48px rows. E2E: `tests/e2e/command-palette.spec.ts` (open+search at 320/768/1280/2560 — button below md, Ctrl+K above — plus the analysis-trigger + mocked live-exchange test; each asserts no horizontal overflow while open).
