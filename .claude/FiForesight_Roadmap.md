...
- Currency-aware price display (Feature 35): instrument currency read from yfinance (`info['currency']`, GBp/minor-unit aware) threaded through /predict + screener/watchlist rows; `formatPrice` helper replaces hardcoded '$' everywhere; analysis-page 'Show in USD' toggle via keyless yfinance FX pairs (backend/fx.py, Redis 1h). Fixes BP.L '$516.50'-is-pence.
