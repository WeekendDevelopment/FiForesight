...
- Market Treemap (Feature 32): Finviz-style stock-level market map — `GET /market/treemap` (screener-universe snapshot + batched 1d changes, Redis 10min) + `MarketTreemap` (Recharts Treemap, cap-weighted tiles, colour by % move, click-to-analyze) as a 'Map' view toggle on /sectors alongside the F23 sector heatmap.
