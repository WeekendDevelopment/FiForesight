...
- Market Treemap (Feature 32): Finviz-style stock-level market map — `GET /market/treemap` (screener-universe snapshot + batched 1d changes, Redis 10 min) + `MarketTreemap` (Recharts Treemap, cap-weighted tiles, colour by % move, click-to-analyze) as a 'Map' view toggle on /sectors alongside the F23 sector heatmap.
- Analysis Navigator (Feature 34): sticky section chip-nav (Overview/Forecast/AI Jury/Trade/Valuation/Data) with IntersectionObserver active-tracking, smooth jump-to, and collapsible section groups persisted in localStorage `fiforesight:analysis:sections`. Collapse keeps cards mounted (no refetch). Fixes the 23-card wall.
...
- Command Palette (Feature 33): global Ctrl/Cmd+K (and '/') palette — fuzzy ticker search (static universe + live multi-exchange symbol search via `GET /symbols/search`, Yahoo keyless: BP NYSE / BP.L London / … with exchange badges) → /analysis?symbol=, page jump (shared NAV_ITEMS), quick actions (theme toggle, watchlist add/remove), recent tickers in localStorage. MUI-native (no cmdk dep); mobile full-width sheet via Search button; the analysis AND landing pages' inline Autocompletes + manual exchange dropdowns were consolidated into it (both search bars are now palette triggers; the redundant sidebar Search button was removed). e2e open+search (all 4 viewports) + live-exchange specs.
...
- Currency-aware price display (LSE & other non-USD exchanges) — the UI hardcodes a `$` on every quote, so a GBp (pence)-denominated LSE name like **BP.L** renders "$516.50" when it's really 516.50 **pence** (~$6.85) — wrong symbol *and* off by ~75×. Fix: read the instrument's real currency from yfinance (`info["currency"]` / `financialCurrency`; yfinance reports LSE in `GBp`) and either (a) label it correctly (`p`/`GBX`, `€`, `¥`, etc.) or (b) offer a "show in USD" toggle that converts via a free FX rate (e.g. yfinance `GBPUSD=X`, handling the pence→pound ÷100 for `GBp`). Applies everywhere a price/target/SMA/high-low is shown (header, `PriceChartCard`, `TradeSetupCard`, `DCFCard`, watchlist/screener tables, treemap). Store/propagate a `currency` field from `/predict` & friends instead of assuming USD. · `[Value: High]` `[Effort: Med]`
